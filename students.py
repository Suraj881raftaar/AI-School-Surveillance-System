...
class StudentDB:
    """SQLite helper for student records."""
    ...
    def create_table(self) -> None:
        ...
        try:
            self.cur.execute(
                """
                CREATE UNIQUE INDEX IF NOT EXISTS idx_students_roll_no
                ON students(roll_no)
    ...
    def add_student(
        ...
        self.cur.execute(
            ...
            (student_name, roll_no, class_name, mobile, image_path)
            VALUES (?, ?, ?, ?, ?)
            """,
            (student_name, roll_no, class_name, mobile, image_path),
        )
    ...
    def update_student(
        ...
        self.cur.execute(
            ...
            WHERE id = ?
            """,
            (student_name, roll_no, class_name, mobile, image_path, student_id),
        )
        self.conn.commit()
    ...
    def update_image(self, student_id: int, image_path: str) -> None:
        self.cur.execute(
    ...
    def get_student(self, student_id: int) -> tuple | None:
        self.cur.execute(
            """
            SELECT id, student_name, roll_no, class_name, mobile, image_path
            FROM students
            WHERE id = ?
            """,
            (student_id,),
...
class StudentsFrame(ttk.Frame):
    ...
    def create_form_panel(self) -> None:
        ...
        self.add_field(left, "Student Name", self.student_name, 1)
        self.add_field(left, "Roll Number", self.roll_no, 2)
        self.add_field(left, "Class", self.class_name, 3)
    ...
    def create_table_panel(self) -> None:
        ...
        search_entry.bind("<Return>", lambda _event: self.search_student())
        ...
        columns = ("ID", "Name", "Roll Number", "Class", "Mobile", "Image")
        ...
        self.table.column("Roll Number", width=120, minwidth=100, anchor="center")
        self.table.column("Class", width=150, minwidth=110)
    ...
    def select_student(self, event=None) -> None:
        ...
        self.student_name.set(student[1] or "")
        self.roll_no.set(student[2] or "")
        self.class_name.set(student[3] or "")
        self.mobile.set(student[4] or "")
        self.image_path.set(student[5] or "")
    ...
    def capture_face(self) -> None:
        if self.selected_id is None:
            messagebox.showwarning("Warning", "Please Select Student First")
            return
        ...
        save_folder = self.get_student_folder(self.selected_id)
    ...
    def get_student_folder(self, student_id: int) -> str:
        return os.path.join(STUDENT_FOLDER, str(student_id))
